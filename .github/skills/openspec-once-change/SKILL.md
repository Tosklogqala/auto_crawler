---
name: openspec-once-change
description: Implement ONE task from an OpenSpec change. Use when the user wants to implement a single task with progress tracking, testing, and git commit.
license: MIT
...
### 3. **Git commit**
   ```bash
   git add .
   git commit -m "task(<change-name>): <task description>"
   ```
   Use conventional commit messages:
   - `task:` for new features
   - `fix:` for bug fixes
...
Git commit: task(<change-name>): <task description>
```
...
[Git commit: "task(keyword-web-crawler): implement keyword management API"]

[Agent] ## Task Completed
Progress: 3/7 tasks complete
Next: Task 4/7 - Implement keyword search functionality

Run `/opsx:once` again to continue.
```
